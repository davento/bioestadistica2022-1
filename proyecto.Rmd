...
```{r, warning=FALSE}
analyze_dist <- function(x, threshold_p = 0.05, threshold_cv = 0.5, verbose = TRUE){
  D <- lillie.test(x)
  print(D)
  p_value <- D[2]
  is_normal <- (p_value > threshold_p)
  cv <- sd(x, na.rm = TRUE)/mean(x, na.rm = TRUE)
  is_parametric <- (cv < threshold_cv)
  if(verbose){
    if(is_normal){
      cat("* Has normal distribution.\n")
    } else {
      cat("* Has not a normal disribution.\n")
    }
    if(is_parametric){
      cat("* Has parametric distribution.\n")
    } else{
      "* Has not a parametric distribution.\n"
    }
  }
